...
router.get('/add-admin', async function (req, res) {
    var exist = await UserModel.findOne({numberID: '9876' });
    if (!exist) {
...
router.post('/register', passport.authenticate('local-register'),
  async function (req, res) {
    if (req.session.passport) {
      res.json({ connect: true, success: req.user.usernameMail, role: req.user.role, myCart: {cart: null} })
    } else
  ...
  async function (req, res) {
    if (req.session.passport) {
      if (req.user.role == 'user') {
        ...
        res.json({ connect: true, success: req.user.usernameMail, role: req.user.role, name: req.user.firstName, myCart: results });
      } else {
